			...
			smartPop.confirm( confirmMessage, function(){
				...
				$.ajax({
					...
					success : function(data, status, jqXHR) {
						...
						smartPop.showInfo(smartPop.INFORM, isEmpty(lineId) ? smartMessage.get('createWebServiceSucceed') : smartMessage.get('setWebServiceSucceed'), function(){
							document.location.href = "web_service.sw";					
						});
					},
					error : function(e) {
						smartPop.closeProgress();
						smartPop.showInfo(smartPop.ERROR, isEmpty(lineId) ? smartMessage.get('createWebServiceError') : smartMessage.get('setWebServiceError'));
					}
				});
			});
		}
	...
	<form name="frmEditWebService" class="form_layout margin_b10 js_validation_required">
		<table>
			<tbody>
				...
				<tr>
					<th><fmt:message key="settings.title.webservice.port"/><span class="essen_n"></span></th>
					<td>
						<select>
							<option></option>
						</select>
				...
				<tr>
					<th><fmt:message key="settings.title.webservice.operation"/><span class="essen_n"></span></th>
					<td>
						<select name="select">
							<option> - </option>
						</select>
				...
				<tr>
					...
					if(webService.getInputVariables()!=null && webService.getInputVariables().length>0){
						int widthVal = 80/webService.getInputVariables().length;
						WebService.Variable[] inputVariables = webService.getInputVariables();
						for(int count=1; count<inputVariables.length+1; count++){
							WebService.Variable inputVariable = inputVariables[count-1]; 
					%>					
