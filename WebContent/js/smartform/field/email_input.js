...
SmartWorks.FormRuntime.EmailInputBuilder.build = function(config) {
	...
	var $email = null;
	if(readOnly){
		$email = $('<div class="form_value form_text_input" style="width:' + valueWidth + '%"></div>').text(value);
	}else{	
		$email = $('<div class="form_value form_text_input" style="width:' + valueWidth + '%"><input type="text" name="' + id + '"' + required + '></div>').attr('value', value);
	}
