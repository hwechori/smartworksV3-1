...
public class PrcProcessCond extends MisObjectCond {
	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(PrcProcessCond.class);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		PrcProcessCond obj = null;
		if (baseObj == null || !(baseObj instanceof PrcProcessCond))
			obj = new PrcProcessCond();
		else
			obj = (PrcProcessCond)baseObj;
	...
	public static PrcProcessCond[] add(PrcProcessCond[] objs, PrcProcessCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		PrcProcessCond[] newObjs = new PrcProcessCond[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static PrcProcessCond[] remove(PrcProcessCond[] objs, PrcProcessCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static PrcProcessCond[] left(PrcProcessCond[] objs, PrcProcessCond obj) {
		...
		if (idx < 1)
			return objs;
		PrcProcessCond[] newObjs = new PrcProcessCond[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static PrcProcessCond[] right(PrcProcessCond[] objs, PrcProcessCond obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
