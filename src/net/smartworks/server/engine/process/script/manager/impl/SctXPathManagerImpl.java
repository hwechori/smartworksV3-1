package net.smartworks.server.engine.process.script.manager.impl;
...
public class SctXPathManagerImpl implements ISctManager {
	...
	public Object execute(String script, Object context) throws SctException {
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("Execute Xpath Script [ " + script + " = " + result + " ( " + context + " ) ]");
			}
			return result;
		} catch (Exception e) {
	...
	private JXPathContext getJXpathContext(Object context) {
		JXPathContext _context = JXPathContext.newContext(context);
		_context.setFunctions(this.functionLibrary);
		return _context;
	}
}
