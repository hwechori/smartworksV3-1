...
public class SwoConfig extends SwoObject {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(SwoConfig.class);
	...
	public static SwoConfig[] add(SwoConfig[] objs, SwoConfig obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		SwoConfig[] newObjs = new SwoConfig[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static SwoConfig[] remove(SwoConfig[] objs, SwoConfig obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static SwoConfig[] left(SwoConfig[] objs, SwoConfig obj) {
		...
		if (idx < 1)
			return objs;
		SwoConfig[] newObjs = new SwoConfig[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static SwoConfig[] right(SwoConfig[] objs, SwoConfig obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
