package net.smartworks.server.engine.config.eventday.model;
...
public class EventDayCond extends MisObjectCond {
	...
	private static Log logger = LogFactory.getLog(EventDayCond.class);
	private static final String NAME = CommonUtil.toName(EventDayCond.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		EventDayCond obj = null;
		if (baseObj == null || !(baseObj instanceof EventDayCond))
			obj = new EventDayCond();
		else 
			obj = (EventDayCond)baseObj;
	...
	public static EventDayCond[] add(EventDayCond[] objs, EventDayCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs !=null)
			size = objs.length;
		EventDayCond[] newObjs = new EventDayCond[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static EventDayCond[] remove(EventDayCond[] objs, EventDayCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static EventDayCond[] left(EventDayCond[] objs, EventDayCond obj) {
		...
		if (idx < 1)
			return objs;
		EventDayCond[] newObjs = new EventDayCond[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static EventDayCond[] right(EventDayCond[] objs, EventDayCond obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
