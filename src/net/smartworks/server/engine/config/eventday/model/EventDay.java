...
public class EventDay extends MisObject {
	...
	private static Log logger = LogFactory.getLog(EventDay.class);
	private static final String NAME = CommonUtil.toName(EventDay.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj)throws Exception {
		if (node == null)
			return null;
		EventDay obj = null;
		if (baseObj == null || !(baseObj instanceof EventDay))
			obj = new EventDay();
		else
			obj = (EventDay)baseObj;
	...
	public static EventDay[] add(EventDay[] objs, EventDay obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if(objs != null)
			size = objs.length;
		EventDay[] newObjs = new EventDay[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static EventDay[] remove(EventDay[] objs, EventDay obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static EventDay[] left(EventDay[] objs, EventDay obj) {
		...
		if (idx < 1)
			return objs;
		EventDay[] newObjs = new EventDay[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static EventDay[] right(EventDay[] objs, EventDay obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for(int i=0; i<objs.length; i++) {
