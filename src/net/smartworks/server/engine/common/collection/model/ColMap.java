...
public class ColMap extends MisObject {
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		ColMap obj = null;
		if (baseObj == null || !(baseObj instanceof ColMap))
			obj = new ColMap();
		else
			obj = (ColMap)baseObj;
	...
	public static ColMap[] add(ColMap[] objs, ColMap obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		ColMap[] newObjs = new ColMap[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static ColMap[] remove(ColMap[] objs, ColMap obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static ColMap[] left(ColMap[] objs, ColMap obj) {
		...
		if (idx < 1)
			return objs;
		ColMap[] newObjs = new ColMap[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static ColMap[] right(ColMap[] objs, ColMap obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
