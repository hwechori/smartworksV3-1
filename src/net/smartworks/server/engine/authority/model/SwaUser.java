...
public class SwaUser extends MisObject {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(SwaUser.class);
	
	protected static final String PREFIX = "Swa";
	private static final String NAME = CommonUtil.toName(SwaUser.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		SwaUser obj = null;
		if (baseObj == null || !(baseObj instanceof SwaUser))
			obj = new SwaUser();
		else
			obj = (SwaUser)baseObj;
	...
	public static SwaUser[] add(SwaUser[] objs, SwaUser obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		SwaUser[] newObjs = new SwaUser[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static SwaUser[] remove(SwaUser[] objs, SwaUser obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static SwaUser[] left(SwaUser[] objs, SwaUser obj) {
		...
		if (idx < 1)
			return objs;
		SwaUser[] newObjs = new SwaUser[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static SwaUser[] right(SwaUser[] objs, SwaUser obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
