...
public class DataCond extends MisObjectCond {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(DataCond.class);

	protected static final String PREFIX = "Data";
	private static final String NAME = CommonUtil.toName(DataCond.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		DataCond obj = null;
		if (baseObj == null || !(baseObj instanceof DataCond))
			obj = new DataCond();
		else
	...
	public static DataCond[] add(DataCond[] objs, DataCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		DataCond[] newObjs = new DataCond[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		return newObjs;
	}
	public static DataCond[] remove(DataCond[] objs, DataCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static DataCond[] left(DataCond[] objs, DataCond obj) {
		...
		if (idx < 1)
			return objs;
		DataCond[] newObjs = new DataCond[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static DataCond[] right(DataCond[] objs, DataCond obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
