...
public class AddressBookCond extends MisObjectCond {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(AddressBookCond.class);
	
	private static final String NAME = CommonUtil.toName(AddressBookCond.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		AddressBookCond obj = null;
		if (baseObj == null || !(baseObj instanceof AddressBookCond))
			obj = new AddressBookCond();
		else
			obj = (AddressBookCond)baseObj;
	...
	public static AddressBookCond[] add(AddressBookCond[] objs, AddressBookCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		AddressBookCond[] newObjs = new AddressBookCond[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		newObjs[i] = obj;
	...
	public static AddressBookCond[] remove(AddressBookCond[] objs, AddressBookCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static AddressBookCond[] left(AddressBookCond[] objs, AddressBookCond obj) {
		...
		if (idx < 1)
			return objs;
		AddressBookCond[] newObjs = new AddressBookCond[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static AddressBookCond[] right(AddressBookCond[] objs, AddressBookCond obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
