...
package net.smartworks.server.engine.basicwork.board.model;
...
public class BoardCond extends MisObjectCond {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(BoardCond.class);

	protected static final String PREFIX = "Noti";
	private static final String NAME = CommonUtil.toName(BoardCond.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		BoardCond obj = null;
		if (baseObj == null || !(baseObj instanceof BoardCond))
			obj = new BoardCond();
		else
			obj = (BoardCond)baseObj;
	...
	public static BoardCond[] add(BoardCond[] objs, BoardCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		BoardCond[] newObjs = new BoardCond[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		return newObjs;
	}
	public static BoardCond[] remove(BoardCond[] objs, BoardCond obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static BoardCond[] left(BoardCond[] objs, BoardCond obj) {
		...
		if (idx < 1)
			return objs;
		BoardCond[] newObjs = new BoardCond[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static BoardCond[] right(BoardCond[] objs, BoardCond obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
